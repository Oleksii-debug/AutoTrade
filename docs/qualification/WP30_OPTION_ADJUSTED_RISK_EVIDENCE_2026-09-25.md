...
## Reconvergence on current main

This successor is semantically reconstructed on exact main `92ca4e0cfd8949645663eeaf092ac2db156ec75a` rather than copying the stale branch. It preserves the newer fail-closed exercise-window contract already in main: exercise style is restricted to AMERICAN/EUROPEAN, holder exercise requires explicit `exercise_opens_at`, and pre-open/missing-window states remain blocked. The WP-30 adjusted-deliverable and risk-evidence delta is layered on top of those rules.
