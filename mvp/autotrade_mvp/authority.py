...
class AllocationAuthoritySnapshot:
    """Current canonical facts used to revalidate one allocation at admission.

    The snapshot is supplied by an AuthorityService-owned resolver, never by
    the admission caller. It adapts existing canonical stores; it is not a
    second financial authority or persistence layer.
    """
...
class AuthorityService:
    ...
    def _allocation_binding_for_admission(
        ...
        *,
        ...
        """Re-resolve and bind one allocation before Transaction A.

        Exact retries reuse the already committed pre-reservation binding
        instead of requiring this command's own reservation mutation to vanish.
        """
        ...
        if existing is not None:
            ...
            if (
                len(risk_events) != 1
                or not isinstance(risk_events[0].get("payload"), Mapping)
            ):
                raise AuthorityConflict(
                    "existing allocation admission evidence is missing"
                )
            persisted = risk_events[0]["payload"].get("allocation_evidence")
        ...
        if set(current.resolved_evidence) != expected_evidence_ids:
            raise AuthorityConflict(
                "allocation authority evidence set is incomplete or ambiguous"
            )

        normalized_provider = _text(
        ...
        if current.provider_id.upper() != normalized_provider:
            raise AuthorityConflict(
                "allocation provider does not match admission provider"
            )
        if current.account_id != normalized_account:
            raise AuthorityConflict(
        ...
        capability_by_symbol = dict(
            allocation_result.capability_snapshot_ids
        )
        bound_capability = capability_by_symbol.get(risk_intent.symbol)
        if (
