...
class HostPrincipal:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.session, str) or not self.session.strip():
            raise ValueError("principal session is required")
...
def header_principal_resolver(
    ...
    return HostPrincipal(actor=actor, session=token)
    ...
    def __init__(
        ...
        *,
        ...
        self.store = JournalBackedHostCommandStore(
            ...
            session_validator=security_boundary.validate_host_session,
        ...
        *,
        ...
        try:
            ...
            if method == "POST" and path == "/api/v1/commands":
                ...
                token = _REQUEST_ORIGIN.set(self.public_origin)
                try:
