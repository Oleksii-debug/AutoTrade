...
surfaces can submit/read/pause/resume/cancel research work without exposing generic
financial job execution.
"""
...
_CONTROL_ROLES = {"OWNER", "RESEARCHER"}
...
class ResearchJobHostService:
    ...
    def _authorized_identity(
        ...
        *,
        ...
        return normalized_actor, authenticated.role
    ...
    def _authorize(
        ...
        *,
        ...
        normalized_actor, _role = self._authorized_identity(
            session=session,
            actor=actor,
            roles=roles,
        )
    ...
    def _authorize_job_control(
        ...
        *,
        ...
        normalized_actor, role = self._authorized_identity(
            session=session,
            actor=actor,
            roles=_CONTROL_ROLES,
        )
        if role == "OWNER":
            return normalized_actor
        ...
        if not isinstance(submitted_by, str) or not secrets.compare_digest(
            ...
            raise PermissionError(
                "Researcher may control only research jobs durably submitted by that subject"
            )
        return normalized_actor
    ...
    def pause(
        ...
        *,
        ...
        self._authorize_job_control(
            job_id,
            session=session,
            actor=actor,
        )
        return self._jobs.pause(job_id)
    ...
    def resume(
        ...
        *,
        ...
        return self._jobs.resume(job_id)
