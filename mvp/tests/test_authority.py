...
class AuthorityTests(unittest.TestCase):
    ...
    def test_confirmation_cannot_authorize_different_allowed_action(self):
        service = AuthorityService()
        service.register_policy(policy())
        service.add_confirmation(
        ...
        result = service.admit(
            admission_id="a-scope", policy_id="p1", intent_hash="same-hash",
            account_id="paper-1", environment="PAPER", instrument="ABC",
            action="ORDER.CANCEL", notional="100", state_version=1,
            risk_admitted=True, now="2026-09-24T18:00:00Z",
            confirmation_id="c-scope",
        )
    ...
    def test_confirmation_cannot_authorize_different_notional(self):
        service = AuthorityService()
        service.register_policy(policy())
        service.add_confirmation(
            confirmation_id="c-notional",
            policy_id="p1",
            intent_hash="same-hash",
            account_id="paper-1",
            environment="PAPER",
        ...
        result = service.admit(
            admission_id="a-notional", policy_id="p1", intent_hash="same-hash",
            account_id="paper-1", environment="PAPER", instrument="ABC",
            action="ORDER.SUBMIT", notional="101", state_version=1,
            risk_admitted=True, now="2026-09-24T18:00:00Z",
            confirmation_id="c-notional",
        )
        self.assertEqual(result.outcome, "REJECTED")
