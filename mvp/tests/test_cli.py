...
class CliTests(unittest.TestCase):
    ...
    def test_main_status_mode_runs_after_function_definition(self):
        with TemporaryDirectory() as directory:
            output = io.StringIO()
    ...
    def test_multi_episode_mode_emits_json_array(self):
        with TemporaryDirectory() as directory:
            ...
            self.assertEqual(payload[-1]["position"], "0E-8")

    def test_semicolon_requires_multi_episode_mode(self):
        with TemporaryDirectory() as directory:
            with patch(
                "sys.argv",
