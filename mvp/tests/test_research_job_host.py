...
class ResearchJobHostServiceTests(unittest.TestCase):
    ...
    def test_researcher_can_enqueue_durable_research_job_and_observer_can_read(self):
        ...
        self.assertEqual(durable["submitted_by"], "researcher")
    ...
    def test_foreign_researcher_cannot_cancel_another_submitters_job(self):
        created, _ = self.enqueue()
    ...
    def test_researcher_controls_only_own_durably_submitted_job(self):
        created, _ = self.enqueue()
        ...
        self.assertEqual(self.jobs.get(created["job_id"])["state"], "PAUSED")
    ...
    def test_owner_may_control_legacy_job_without_submitter_but_researcher_may_not(self):
        legacy, _ = self.jobs.enqueue(
            kind="research.replay",
            dedupe_key="legacy-ownerless",
            input_hashes=[digest("dataset")],
            resource_budget={"wall_seconds": 60},
            lease_requeueable=True,
        )
