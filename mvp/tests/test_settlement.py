...
class EconomicSettlementCapitalTests(unittest.TestCase):
    ...
    def rule(
        ...
        *,
        ...
        return SettlementRuleBinding(
            ...
            evidence_refs=("instrument:ABC", f"rule:{version}"),
        )
    ...
    def test_margin_buying_power_is_separate_evidence_not_settled_cash(self):
        ...
        resources = capital.reservation_resources()
    ...
    def test_settlement_rule_for_different_instrument_version_fails_closed(self):
        ...
        wrong_rule = SettlementRuleBinding(
            ...
            effective_from=date(2026, 9, 1),
            effective_to=None,
            evidence_refs=("instrument:OTHER", "rule:wrong-instrument:1"),
        )
        with self.assertRaisesRegex(
            SettlementConflict,
            "instrument version does not match",
        ):
            equity_cash_obligation_from_transaction(
                sold,
                obligation_id="wrong-instrument-ob",
                instrument="ABC",
                settlement_currency="USD",
                settlement_date=date(2026, 9, 25),
                rule_binding=wrong_rule,
            )
    ...
    def test_reservation_authority_uses_available_cash_not_trade_date_cash(self):
        ...
        with self.assertRaisesRegex(InsufficientAvailable, "Insufficient"):
            reservations.reserve_from_capital(
    ...
    def test_overdue_unknown_capital_is_rejected_by_reservation_authority(self):
        ...
        self.assertTrue(capital.blocks_new_risk)
        with self.assertRaisesRegex(InsufficientAvailable, "blocks new risk"):
    ...
    def test_fx_cash_legs_use_same_settlement_availability_semantics(self):
        book = self.funded_book()
        fx = book_fx_exchange(
            transaction_id="fx-1",
            cause_event_id="fx-event-1",
            sold_currency="USD",
            sold_amount="100",
            bought_currency="EUR",
            bought_amount="90",
        ...
        book.append(fx)
        usd_rule = self.rule()
        eur_rule = SettlementRuleBinding(
            rule_id="fx-eur-settlement",
            rule_version="1",
            scope=self.scope,
            instrument_version="EURUSD@1",
            settlement_currency="EUR",
            effective_from=date(2026, 9, 1),
            effective_to=None,
            evidence_refs=("instrument:EURUSD@1", "rule:fx-eur:1"),
        )
        usd = cash_settlement_obligation_from_transaction(
            fx,
            obligation_id="fx-usd",
            currency="USD",
            trade_date=date(2026, 9, 24),
            settlement_date=date(2026, 9, 25),
            component_id="FX_SOLD_LEG",
            instrument_version="ABC",
            rule_binding=usd_rule,
        )
        eur = cash_settlement_obligation_from_transaction(
            fx,
            obligation_id="fx-eur",
            currency="EUR",
            trade_date=date(2026, 9, 24),
            settlement_date=date(2026, 9, 25),
            component_id="FX_BOUGHT_LEG",
            instrument_version="EURUSD@1",
            rule_binding=eur_rule,
        )
        settlement = SettlementBook.from_economic_book(
            economic_book=book,
            obligations=(usd, eur),
        ...
        self.assertEqual(book.cash("USD"), Decimal("900"))
        self.assertEqual(book.cash("EUR"), Decimal("90"))
        self.assertEqual(settlement.snapshot("USD").settled_cash, Decimal("1000"))
        self.assertEqual(settlement.snapshot("USD").unsettled_payable, Decimal("100"))
        self.assertEqual(settlement.available_to_spend("USD"), Decimal("900"))
        self.assertEqual(settlement.snapshot("EUR").settled_cash, Decimal("0"))
        self.assertEqual(settlement.snapshot("EUR").unsettled_receivable, Decimal("90"))
        self.assertEqual(settlement.available_to_spend("EUR"), Decimal("0"))

    def test_one_economic_fill_cannot_be_bound_as_two_full_cash_obligations(self):
        book = self.funded_book()
        sold = self.fill(transaction_id="sell-duplicate-binding", side="SELL")
        book.append(sold)
        first = self.bound(sold, obligation_id="ob-binding-a")
        second = SettlementObligation(
            obligation_id="ob-binding-b",
            cause_event_id=sold.cause_event_id,
            currency="USD",
            amount="100",
            trade_date=date(2026, 9, 24),
            settlement_date=date(2026, 9, 25),
            component_id="SECONDARY",
            source_transaction_id=sold.transaction_id,
            rule_binding=self.rule(),
        )
        with self.assertRaisesRegex(
            SettlementConflict,
            "multiple full settlement obligations",
        ):
