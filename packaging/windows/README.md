...
## Velopack packaging executor

`tools/build_windows_velopack.py` is the single selected WP-50 installer packaging executor. It consumes the canonical release ZIP and its exact installer-input manifest, verifies their identity match, materializes only the verified payload into an isolated temporary pack directory, and invokes the repository-pinned `vpk 1.2.158` tool. Ambient `VPK_*` environment options are stripped so a CI or operator environment cannot silently change product identity, output location, signing parameters or package policy.

The first qualified path is intentionally narrow: Windows x64, per-user `Setup.exe` enforced with `--instLocation PerUser`, no MSI, no portable bundle, no delta package and no default file exclusions. Velopack self-update checks are disabled for the packaging invocation. Generated bytes are first created in a temporary output directory, validated as the expected Setup/full-package/release-index family, then durably published with SHA-256 sidecars. A deterministic `autotrade-velopack-build.json` inventory is published last and explicitly records `UNSIGNED_REQUIRES_WP64`, `release_eligible=false` and no trading authority. It is evidence for the downstream signing/qualification boundary, not a release approval.
