...
class ReturnThresholdBaseline:
    ...
    def restore(cls, snapshot: str) -> "ReturnThresholdBaseline":
        ...
        if version == 2:
            expected.add("observations_by_id")
        if set(payload) != expected or not isinstance(payload["history"], dict):
            raise ValueError("strategy snapshot structure is invalid")
        if version == 2 and not isinstance(payload["observations_by_id"], dict):
            raise ValueError("strategy observation identity snapshot is invalid")
        strategy = cls(
        ...
        if version == 2:
            ...
            for event_id, row in sorted(payload["observations_by_id"].items()):
                ...
                if existing is not None and existing != observation:
                    raise ValueError("observation identity snapshot conflicts with retained history")
            ...
            if not retained_ids.issubset(declared_ids):
                raise ValueError("observation identity snapshot is missing retained history")
        return strategy
