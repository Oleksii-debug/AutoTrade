...
class HistoricalVintageTests(unittest.TestCase):
    ...
    def test_point_in_time_view_orders_by_evidenced_availability(self):
        earlier_source = {
            ...
            "payload": {"price": "100"},
            "quality_flags": [],
            "raw_evidence_ref": evidence("2026-01-02T00:00:00Z"),
        }
    ...
    def test_market_event_requires_availability_basis_and_raw_evidence(self):
        base = {
            "event_id": str(uuid4()),
            "instrument_version": "instrument:v1",
            "kind": "TRADE",
        ...
        with self.assertRaisesRegex(HistoricalDataError, "availability_basis"):
            point_in_time_market_events(
                [{**base, "availability_basis": "", "raw_evidence_ref": evidence("2026-01-01T10:00:02Z")}],
                cutoff,
            )
        with self.assertRaisesRegex(HistoricalDataError, "raw_evidence_ref"):
            point_in_time_market_events(
                [{**base, "availability_basis": "provider", "raw_evidence_ref": "not-an-object"}],
                cutoff,
            )
    ...
    def test_revision_lineage_cannot_move_availability_backwards(self):
        event_id = str(uuid4())
        common = {
            "event_id": event_id,
            "instrument_version": "instrument:v1",
        ...
        revision_two = {
            **common,
            ...
            "ingested_at": "2026-01-01T10:04:01Z",
            "revision": "2",
            "payload": {"price": "11"},
        }
        with self.assertRaisesRegex(HistoricalConflict, "cannot move backwards"):
            point_in_time_market_events(
                [revision_one, revision_two],
                datetime(2026, 1, 2, tzinfo=timezone.utc),
            )

    def test_revision_lineage_cannot_change_source_identity(self):
        event_id = str(uuid4())
        common = {
            "event_id": event_id,
            "instrument_version": "instrument:v1",
