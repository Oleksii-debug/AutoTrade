...
class ContractVersionGuardTests(unittest.TestCase):
    ...
    def test_python_runtime_artifacts_do_not_change_contract_surface(self):
        with TemporaryDirectory() as left, TemporaryDirectory() as right:
            write_tree(Path(left))
            write_tree(Path(right))
            cache = Path(right) / "contracts" / "bindings" / "python" / "__pycache__"
            cache.mkdir(parents=True)
            (cache / "common_scalars.cpython-312.pyc").write_bytes(b"generated bytecode")
            (Path(right) / "contracts" / "stray.pyc").write_bytes(b"generated bytecode")
            self.assertEqual(evaluate(Path(left), Path(right)), [])

    def test_new_non_runtime_contract_file_still_requires_version_increment(self):
        with TemporaryDirectory() as left, TemporaryDirectory() as right:
            write_tree(Path(left))
            write_tree(Path(right))
            (Path(right) / "contracts" / "new-contract.txt").write_text(
