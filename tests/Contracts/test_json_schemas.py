...
class ContractSchemaTests(unittest.TestCase):
    ...
    def test_fixture_manifest_is_complete_and_valid(self):
        ...
        for entry in entries:
            with self.subTest(fixture=entry["path"]):
                self.assertIn(entry["expected"], {"valid", "invalid"})
                self.assertIn(entry["schema"], self.schemas)
                fixture = json.loads((FIXTURES / entry["path"]).read_text(encoding="utf-8"))
                schema = self.schemas[entry["schema"]]
                validator = Draft202012Validator(
                    {"$ref": f"{schema['$id']}#/$defs/{entry['definition']}"},
                    registry=self.registry,
                    format_checker=FormatChecker(),
                )
    ...
    def test_ui_command_scope_is_required_and_environment_is_canonical(self):
        valid = json.loads((FIXTURES / "ui-command.valid.json").read_text())
        ...
        self.assertEqual(valid["account_id"], "acct-paper-1")
        self.assertEqual(valid["environment"], "PAPER")

        unscoped = dict(valid)
        unscoped.pop("account_id")
        self.assertFalse(validator.is_valid(unscoped))

        without_environment = dict(valid)
        without_environment.pop("environment")
        self.assertFalse(validator.is_valid(without_environment))

        bad_environment = dict(valid)
        bad_environment["environment"] = "PRODUCTION"
        self.assertFalse(validator.is_valid(bad_environment))
...
, python_anchor)
        ...
        typescript_match = re.search(
            r'(?m)^export declare const CONTRACT_VERSION:\s*"([^"]+)";\s*
        openapi_path = ROOT / self.contract_manifest["openapi"]["path"]
        openapi = openapi_path.read_text(encoding="utf-8")
        self.assertRegex(openapi, r"(?m)^openapi:\s*3\.1\.0\s*$")
        info_version = re.search(r"(?m)^\s{2}version:\s*([^\s#]+)\s*$", openapi)
