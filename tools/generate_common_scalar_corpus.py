...
The candidate values are intentionally explicit review vectors. Acceptance is never
hand-authored: every expected result is derived from common.schema.json.  CI runs
--check so a schema change cannot silently leave the cross-language corpus stale.
"""
...
# Explicit adversarial vectors are part of the reviewed contract surface.  Keep names
# stable so failures identify the same semantic boundary across Python/C#/TypeScript.
CANDIDATES: tuple[tuple[str, str, object], ...] = (
    ...
    ("currency-max-length", "CurrencyId", "x" * 32),
    ("currency-too-long", "CurrencyId", "x" * 33),
    ("unit-simple", "UnitId", "shares"),
    ("unit-path", "UnitId", "contract/BTC-USD"),
    ("unit-space", "UnitId", "base units"),
    ("unit-max-length", "UnitId", "x" * 64),
